...
import { getSitumJwt, isSitumUrl } from "./situm";
...
function MapView(props: {
  ...
  const [tilesetMaxZoom, setTilesetMaxZoom] = createSignal<number>(22);
  ...
  const addTileset = async (tileset: Tileset) => {
    setTilesetMaxZoom(await tileset.getMaxZoom());
    ...
    const source = await tileset.getMaplibreSource();
    if (source.type === "vector") {
      map.addSource("tileset", source);
  ...
  onMount(async () => {
    ...
    map.on("mousemove", async (e) => {
      ...
      const z = Math.max(
        0,
        Math.min(tilesetMaxZoom(), Math.floor(currentZoom)),
      );
