...
function SitumAuthButton(props: {
  ...
  const reloadSitumTileset = () => {
    const url = props.tileset()?.getStateUrl();
    if (url && isSitumUrl(url)) {
      props.setTileset(tilesetFromString(url));
    }
  };

  const openDialog = () => {
    setDraft(situmApiKey());
    setOpen(true);
  };

  const save = () => {
    setSitumApiKey(draft().trim());
    setOpen(false);
    reloadSitumTileset();
  };
  ...
  return (
    <>
      <button
        type="button"
        class="px-2 ml-2 text-sm btn-secondary cursor-pointer whitespace-nowrap"
        onClick={openDialog}
      >
        {situmApiKey() ? "Situm API key ✓" : "Situm API key"}
      </button>
      <Show when={open()}>
        <div class="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4">
          <div class="app-bg app-border rounded p-4 w-full max-w-md shadow-lg">
            <h2 class="text-lg font-semibold mb-2">Situm API key</h2>
            <p class="text-sm app-text-light mb-3">
              Required to load PMTiles or TileJSON hosted on situm.com. Your key
              is stored in this browser only.
            </p>
            <input
              class="app-border w-full px-2 py-1 mb-3"
              type="password"
              autocomplete="off"
              placeholder="API key"
              value={draft()}
            ...
            <div class="flex gap-2 justify-end">
              <Show when={situmApiKey()}>
                <button
                  type="button"
                  class="px-3 py-1 btn-secondary cursor-pointer"
                  onClick={signOut}
                >
                  clear key
                </button>
              ...
              <button
                type="button"
                class="px-3 py-1 btn-primary cursor-pointer"
                onClick={save}
              >
                save
              </button>
